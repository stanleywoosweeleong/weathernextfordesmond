...
3 farms owned by Desmond Chin across Perak and Terengganu. Multilingual interface
...
This release migrates the Desmond build from the older forecast-only build onto
...
```
https://stanleywoosweeleong.github.io/weathernextfordesmond/
```
...
|---|---|---|---|---|---|
| Kampung Chapu | 甘榜查布 | 5.17750, 102.89500 | Terengganu | open plain | durian |
| Kuala Berang | 瓜拉伯浪 | 5.17250, 102.53333 | Terengganu | riverine | durian |
| Sungkai | 宋溪 | 3.97889, 101.36028 | Perak | open plain | durian |
...
The app also seeds a default user display name (**陈联铕 / Desmond Chin**),
which stays editable via **Edit Name** in the app.

This build carries the seed version **`dm-arch1`** (the lowland-architecture
...
```
wnext-weathernextfordesmond-202606082359
```
...
- **Storage namespace:** `weathernextfordesmond__*` keys in `localStorage`, isolated
  from other WeatherNext regional builds so data never collides.
- **Cloud sync:** Firebase, namespaced under `appId: wnext-ag-v41-weathernextfordesmond`.
...
Desmond's 3 farms span **two states** (Perak and Terengganu), so the AI briefing
prompt is deliberately **location-name-free**: it gives the model each farm's
coordinates, elevation, terrain zone and live weather, but never asserts a
single region name — keeping advice accurate for every farm regardless of
state. The boot screen background (cream `#f0dcc1`) matches the icon.
